...
from tocketry.core.time.base import TimePeriod, always
...
class Cron(TimePeriod):
    ...
    def _get_period_from_expr(
        self, cls, expression: str, conv: Callable = None, default=always
    ):
        ...
        for expr in exprs:
            ...
            if "-" in expr:
                ...
                # At
                step_period = (
                    cls.create_range(step=int(step)) if step is not None else always
                )
    ...
    def get_subperiod(self):
        ...
        day_of_week = self._get_period_from_expr(
            TimeOfWeek, self.day_of_week, conv=self._convert_day_of_week
        )
