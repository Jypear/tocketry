...
from tocketry.conditions.parameter import IsEnv, ParamExists
from tocketry.conditions.scheduler import SchedulerCycles, SchedulerStarted
from tocketry.conditions.task import (
    DependFailure,
    DependFinish,
    DependSuccess,
    TaskFailed,
...
from tocketry.conditions.time import IsPeriod
...
from tocketry.core.condition import Not
from tocketry.session import Session
from tocketry.time import TimeDelta
from tocketry.time.construct import get_after, get_before, get_between, get_full_cycle
...
def _set_task_has_parsing():
    ...
    for action, cls in clss:
        ...
        for prefix in ("", r"task '(?P<task>.+)' "):
            cond_parsers.update(
                {
                    re.compile(rf"{prefix}has {action}"): cls,
                    re.compile(
                        rf"{prefix}has {action} (?P<type_>this month|this week|today|this hour|this minute) (?P<span_type>starting) (?P<start>.+)"
                    ): func,
                    re.compile(
                        rf"{prefix}has {action} (?P<type_>this month|this week|today|this hour|this minute) (?P<span_type>between) (?P<start>.+) and (?P<end>.+)"
                    ): func,
                    re.compile(
                        rf"{prefix}has {action} (?P<type_>this month|this week|today|this hour|this minute) (?P<span_type>after) (?P<start>.+)"
                    ): func,
                    re.compile(
                        rf"{prefix}has {action} (?P<type_>this month|this week|today|this hour|this minute) (?P<span_type>before) (?P<end>.+)"
                    ): func,
                    re.compile(
                        rf"{prefix}has {action} (?P<type_>this month|this week|today|this hour|this minute)"
                    ): func,
                    re.compile(
                        rf"{prefix}has {action} (?P<type_>this month|this week|today|this hour|this minute) (?P<span_type>on) (?P<start>.+)"
                    ): func,
                    re.compile(
                        rf"{prefix}has {action} (in )?past (?P<past>.+)"
                    ): partial(func, span_type="past"),
                }
            )
...
def _set_scheduler_parsing():
    ...
    cond_parsers.update(
        {
            re.compile(r"scheduler has run over (?P<past>.+)"): partial(
                func, span_type="past", inverse=True
            ),
            re.compile(r"scheduler started (?P<past>.+) ago"): partial(
                func, span_type="past"
            ),
            re.compile(
                r"scheduler has more than (?P<__gt__>[0-9]+) cycles"
            ): SchedulerCycles.from_magic,
            re.compile(
                r"scheduler has less than (?P<__lt__>[0-9]+) cycles"
            ): SchedulerCycles.from_magic,
            re.compile(
                r"scheduler has (?P<__eq__>[0-9]+) cycles"
            ): SchedulerCycles.from_magic,
        }
    )
    ...
    cond_parsers.update(
        {
            ...
            re.compile(
                r"after tasks '(?P<depend_tasks>.+)'( succeeded)?"
            ): DependSuccess._parse_multi_all,
            re.compile(
                r"after any tasks '(?P<depend_tasks>.+)'( succeeded)?"
            ): DependSuccess._parse_multi_any,
            re.compile(r"after task '(?P<depend_task>.+)' failed"): DependFailure,
            re.compile(
                r"after tasks '(?P<depend_tasks>.+)' failed"
            ): DependFailure._parse_multi_all,
            re.compile(
                r"after any tasks '(?P<depend_tasks>.+)' failed"
            ): DependFailure._parse_multi_any,
            re.compile(r"after task '(?P<depend_task>.+)' finished"): DependFinish,
            re.compile(
                r"after tasks '(?P<depend_tasks>.+)' finished"
            ): DependFinish._parse_multi_all,
            re.compile(
                r"after any tasks '(?P<depend_tasks>.+)' finished"
            ): DependFinish._parse_multi_any,
        }
    )
    ...
    cond_parsers.update(
        {
            ...
            re.compile(
                r"param '(?P<key>.+)' is '(?P<value>.+)'"
            ): ParamExists._from_key_value,
        }
    )
