...
def test_app_defaults(tmpdir):
    ...
    assert app.session.config.task_logger_basename == "tocketry.task"
    assert app.session.config.scheduler_logger_basename == "tocketry.scheduler"
...
def test_deprecated():
    ...
    with pytest.warns(DeprecationWarning):
        app = Tocketry(config={"task_execution": "main"})
...
def test_app_settings():
    ...
    # Test recommended
    app = Tocketry(execution="main", task_priority=10)
    assert app.session.config.execution == "main"

    # Test with conf
    app = Tocketry(config=Config(execution="main", task_priority=10))
    assert app.session.config.execution == "main"

    # Test with conf dict
    app = Tocketry(config=dict(execution="main", task_priority=10))
    assert app.session.config.execution == "main"
...
def test_app_run():
    ...
    def do_after(
        arg=Return("do_daily"),
        session_arg=Arg("my_arg"),
        func_arg=Arg("my_func_arg"),
        func_arg_2=FuncArg(lambda: "my val"),
    ):
    ...
    assert set(task.name for task in session.tasks) == {
        "do_daily",
        "do_after",
        "do_fail",
        "never done",
