...
def script_files(tmpdir):
    ...
    copy_file_to_tmpdir(
        tmpdir,
        source_file="succeeding_script.py",
        target_path="scripts/succeeding_script.py",
    )
    copy_file_to_tmpdir(
        tmpdir, source_file="failing_script.py", target_path="scripts/failing_script.py"
    )
    copy_file_to_tmpdir(
        tmpdir,
        source_file="parameterized_script.py",
        target_path="scripts/parameterized_script.py",
    )
    copy_file_to_tmpdir(
        tmpdir,
        source_file="parameterized_kwargs_script.py",
        target_path="scripts/parameterized_kwargs_script.py",
    )
    copy_file_to_tmpdir(
        tmpdir,
        source_file="syntax_error_script.py",
        target_path="scripts/syntax_error_script.py",
    )
...
def set_loggers():
    ...
    formatter = logging.Formatter(
        "%(asctime)s - %(action)s - %(name)s - %(levelname)s - %(message)s"
    )
...
def mock_datetime_now(monkeypatch):
    ...
    def wrapper(dt):
        ...
        mockdatetime._freezed_datetime = mockdatetime(
...
def mongo_conn_str():
    conf_path = Path("tocketry.test/private.yaml")
