...
def test_creating_child(session):
    FuncTask(
        run_creating_child, name="task_1", start_cond=AlwaysTrue(), session=session
    )

    session.config.tasks_as_daemon = False
    session.config.shut_cond = (TaskStarted(task="task_1") >= 1) | ~SchedulerStarted(
        period=TimeDelta("1 second")
    )
