...
def test_multilaunch_terminate_end_cond(execution, session):
    ...
    logger = task.logger
    logs = [
        {"task_name": rec.task_name, "action": rec.action, "run_id": rec.run_id}
        for rec in logger.filter_by()
    ]
    assert logs == [
...
def test_set_run_id(where, func, session):
    ...
    logger = task.logger
    logs = [
        {"task_name": rec.task_name, "action": rec.action} for rec in logger.filter_by()
    ]
    assert logs == [
    ...
    assert logs == [
        {
            "task_name": "task",
            "action": "run",
            "run_id": '{"report_date": "2022-01-01"}',
        },
        {
            "task_name": "task",
            "action": "run",
            "run_id": '{"report_date": "2022-01-02"}',
        },
        {
            "task_name": "task",
            "action": "run",
            "run_id": '{"report_date": "2022-01-03"}',
        },
        {
            "task_name": "task",
            "action": "success",
