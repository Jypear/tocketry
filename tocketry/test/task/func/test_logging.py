...
@pytest.mark.parametrize(
    "last_status", ["run", "success", "fail", "inaction", "terminate"]
)
def test_set_cached_in_init(session, optimized, last_status):
    ...
    for action, created in times.items():
        repo.add(MinimalRecord(task_name="mytask", action=action, created=created))
    # Add one extra (which should be used instead of what we did above)
    times[last_status] = 1752958800.0
    repo.add(
        MinimalRecord(task_name="mytask", action=last_status, created=1752958800.0)
    )

    # Creating the task
    task = FuncTask(lambda: None, execution="main", name="mytask", session=session)
    task.set_cached()
    for action, created in times.items():
...
def test_running(session):
    task = FuncTask(lambda: None, execution="main", session=session)
    task.log_running()
...
def test_handle(session):
    def create_record(action, task_name):
        ...
        record = create_task_record(
            ...
            name="tocketry.task._process",
        )
        return record

    task = FuncTask(lambda: None, execution="main", name="a task", session=session)
    ...
    records = [record.model_dump(exclude={"created"}) for record in records]
    assert [
...
def test_without_handlers(session):
    ...
    # assert str(warns[0].message) == "Logger 'tocketry.task.test' for task 'task 1' does not have ability to be read. Past history of the task cannot be utilized."
...
def test_without_handlers_status_warnings(session):
    ...
    with pytest.warns(UserWarning) as warns:
        task = FuncTask(
            ...
            name="task 1",
            start_cond="always true",
            logger_name="tocketry.task.test",
            execution="main",
            session=session,
        )
    ...
    # Test warnings
    expected_warnings = [
        "Logger 'tocketry.task.test' for task 'task 1' does not have ability to be read. Past history of the task cannot be utilized.",
    ...
    assert list(str(w.message) for w in warns) == [
        "Logger tocketry.task cannot be read. Logging is set to memory. To supress this warning, please set a handler that can be read (redbird.logging.RepoHandler)"
    ]
...
def test_process_no_double_logging(session):
    ...
    # tocketry.task logger (Memory and Stream in this order)
    ...
    task = FuncTask(run_success, name="a task", execution="process", session=session)
    ...
    proc = multiprocessing.Process(
        target=task._run_as_process,
        args=(None, None, task_run, log_queue, session.config, []),
        daemon=None,
    )
    task_run.task = proc
    proc.start()
    ...
    assert (
        len(records) >= 2
    )  # Testing not too few records (else not double logging bug)
    assert (
    ...
    assert (
        len(recs) == 0
    ), "Double logging. The log file is not empty before handling the records. Process bypasses the queue."
    ...
    ), "Double logging. Bug in Task.log_record probably."
