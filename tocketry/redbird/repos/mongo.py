...
from tocketry.redbird.base import BaseResult, BaseRepo
from tocketry.redbird.exc import KeyFoundError
from tocketry.redbird.oper import (
    Between,
    GreaterEqual,
    GreaterThan,
    In,
...
from tocketry.redbird.packages import pymongo
...
class MongoRepo(TemplateRepo):
    ...
    def insert(self, item):
        ...
        try:
            ...
            raise KeyFoundError(
                f"Document {self.get_field_value(item, self.id_field)} already exists."
            ) from exc
    ...
    def upsert(self, item):
        ...
        doc = self.item_to_data(item)
        col.update_one(
            {"_id": self.get_field_value(item, self.id_field)},
            {"$set": doc},
            upsert=True,
        )
    ...
    def get_collection(self) -> "Collection":
        ...
        col_name = (
            self.model.__colname__
            if hasattr(self.model, "__colname__")
    ...
    def _format_dict(self, item: dict) -> BaseModel:
        try:
            ...
            raise ValidationError(
                f"Formatting for {item[self.id_field]} failed."
            ) from exc
    ...
    def query_data_limit(self, query, n: int):
        "Get n first items"
        col = self.get_collection()
        return [self.data_to_item(item) for item in col.find(query).limit(n)]
        ...
        if isinstance(value, Operation):
            ...
            if type_ in self.__operators__:
                ...
                raise NotImplementedError(
                    "MongoRepo does not yet support operator: {type_}"
                )
        else:
            return value
