...
from tocketry.redbird.packages import requests
...
from tocketry.redbird.utils.importing import get_import_error
...
class RESTRepo(TemplateRepo):
    ...
    def insert(self, item):
        ...
        self._request("POST", self.url, json=json)
    ...
    def _request(self, *args, **kwargs):
        page = self.session.request(*args, **kwargs, headers=self.headers)
        return page
    ...
    def format_query(self, query: dict) -> str:
        ...
        id = (
            query.pop(self.id_field)
            if query is not None and self.id_field in query
            else None
        )
