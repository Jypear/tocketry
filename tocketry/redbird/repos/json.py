...
from tocketry.redbird.base import Data, Item
from tocketry.redbird.templates import TemplateRepo
from tocketry.redbird.exc import KeyFoundError
from tocketry.redbird.oper import Operation
from tocketry.redbird.dummy import DummySession
from tocketry.redbird.utils.query import delete_items, read_items, update_items

from tocketry.redbird.utils.query import QueryMatcher
...
class JSONDirectoryRepo(TemplateRepo):
    ...
    def query_update(self, query: dict, values: dict):
        ...
        new_items = update_items(
            self, self.read_data(filename_query), query, values, return_="updated"
        )
        self.write_files(new_items)

    def query_delete(self, query: dict):
        filename_query = query.get(self.id_field, None)
        new_items = delete_items(
            self, self.read_data(filename_query), query, return_="deleted"
        )
