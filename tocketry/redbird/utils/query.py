...
from tocketry.redbird.exc import ConversionWarning, _handle_conversion_error
...
class QueryMatcher:
    ...
    def __contains__(self, item):
        ...
        return all(
            val.evaluate(self.get_value(item, key))
            if isinstance(val, Operation)
            else self.get_value(item, key) == val
...
def update_items(
    ...
    query: dict,
    values: Dict,
    return_: Literal["all", "updated"] = "all",
) -> List[Item]:
    query = QueryMatcher(query, value_getter=repo.get_field_value)
    for data in reader:
        item = repo.data_to_item(data)
...
def delete_items(
    ...
    reader: Generator,
    query: dict,
    return_: Literal["remained", "deleted"] = "remained",
) -> List[Item]:
    query = QueryMatcher(query, value_getter=repo.get_field_value)
    for data in reader:
        item = repo.data_to_item(data)
