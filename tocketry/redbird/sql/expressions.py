...
from tocketry.redbird.packages import sqlalchemy, import_exists
    ...
    def to_query(self):
        ...
        if self.is_multi_item():
            ...
            for item in keys:
                ...
                item_query = [
                    self._to_equal(primary_keys[i], key) for i, key in enumerate(item)
                ]
...
class Table:
    ...
    .. code-block:: python
        ...
        from tocketry.redbird.sql import Table
    ...
    def __getitem__(
        self,
        keys: Union[SINGULAR, Tuple[SINGULAR], List[Union[SINGULAR, Tuple[SINGULAR]]]],
    ...
    def select(
        self,
        qry: Union[str, dict, "sqlalchemy.sql.ClauseElement", None] = None,
        ...
        if isinstance(qry, str):
            ...
            qry, (sqlalchemy.sql.Selectable, sqlalchemy.sql.elements.TextClause)
        ):
            statement = qry
        else:
            if columns is None:
                ...
                columns = [self.object.columns[col] for col in columns]
            where = qry
    ...
    def update(
        self, where: Union[dict, "sqlalchemy.sql.ClauseElement"], values: dict
    ) -> int:
    ...
    def _get_types(self) -> Dict[str, Callable]:
        ...
        return {
            col_name: partial(to_native, sql_type=col.type, nullable=col.nullable)
        ...
        if origin is not None:
            ...
            if origin is typing.Union:
                ...
                if len(args) > 2 or (len(args) == 2 and not has_none_type):
                    raise TypeError(
                        f"Union has more than one optional type: {str(cls)}. Cannot define SQL data type"
                    )
                ...
                if typing.get_origin(cls) is Literal:
                    ...
                    for nested_arg in nested_args:
                        if not isinstance(nested_arg, type_):
                            raise TypeError(
                                f"Literal Values are not same types: {str(cls)}. Cannot define SQL data type"
                            )
                    cls = type_
    ...
    def create(
        self, columns: Union[List["Column"], Mapping[str, Type]], exist_ok=False
    ):
        """Create the table
        ...
        if isinstance(columns, Mapping):
            ...
            columns = [
                sqlalchemy.Column(col, type_=sqlalchemy.String())
    ...
    def create_from_model(self, model: BaseModel, primary_column=None):
        ...
        sql_cols = [
            sqlalchemy.Column(
                ...
                primary_key=name in primary_column
...
def to_expression(qry: dict, table=None):
    ...
    for column_name, oper_or_value in qry.items():
        column = (
            ...
            else sqlalchemy.Column(column_name)
        )
